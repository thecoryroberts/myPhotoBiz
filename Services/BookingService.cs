    ...
    public class BookingService : IBookingService
    {
        ...
        public async Task<PhotoShoot> ConvertToPhotoShootAsync(int bookingId)
        {
            ...
            try
            {
                ...
                // Create PhotoShoot with package and booking reference
                ...
                // Create invoice items from package if available
                if (request.ServicePackage != null)
                {
                    // Main package item
                    invoice.InvoiceItems.Add(new InvoiceItem
                    {
                        Description = $"{request.ServicePackage.Name} - {request.ServicePackage.Category} Package",
                        Quantity = 1,
                        UnitPrice = request.ServicePackage.EffectivePrice
                    });

                    // Add package features as line items for transparency
                    if (request.ServicePackage.IncludedPhotos > 0)
                    {
                        invoice.InvoiceItems.Add(new InvoiceItem
                        {
                            Description = $"Includes {request.ServicePackage.IncludedPhotos} edited photos",
                            Quantity = 1,
                            UnitPrice = 0 // Included in package
                        });
                    }

                    if (request.ServicePackage.IncludesDigitalGallery)
                    {
                        invoice.InvoiceItems.Add(new InvoiceItem
                        {
                            Description = "Digital gallery access",
                            Quantity = 1,
                            UnitPrice = 0 // Included in package
                        });
                    }

                    if (request.ServicePackage.IncludesPrints && request.ServicePackage.NumberOfPrints > 0)
                    {
                        invoice.InvoiceItems.Add(new InvoiceItem
                        {
                            Description = $"Includes {request.ServicePackage.NumberOfPrints} prints",
                            Quantity = 1,
                            UnitPrice = 0 // Included in package
                        });
                    }

                    if (request.ServicePackage.IncludesAlbum)
                    {
                        invoice.InvoiceItems.Add(new InvoiceItem
                        {
                            Description = "Photo album included",
                            Quantity = 1,
                            UnitPrice = 0 // Included in package
                        });
                    }

                    if (request.ServicePackage.NumberOfLocations > 1)
                    {
                        invoice.InvoiceItems.Add(new InvoiceItem
                        {
                            Description = $"Up to {request.ServicePackage.NumberOfLocations} locations",
                            Quantity = 1,
                            UnitPrice = 0 // Included in package
                        });
                    }

                    if (request.ServicePackage.OutfitChanges > 0)
                    {
                        invoice.InvoiceItems.Add(new InvoiceItem
                        {
                            Description = $"Up to {request.ServicePackage.OutfitChanges} outfit changes",
                            Quantity = 1,
                            UnitPrice = 0 // Included in package
                        });
                    }
                ...
                {
                    // No package - create a generic line item
                    invoice.InvoiceItems.Add(new InvoiceItem
                    {
                        Description = $"Photography Services - {request.EventType}",
                        Quantity = 1,
                        UnitPrice = shootPrice
                    });
                }
